// TODO: compositor 에는 trait만 남기고, 특정 API와의 연동은 다른 crate로 분리하기
mod wgpu_layer;

use futures::executor::block_on;
use wgpu::util::DeviceExt;
pub use wgpu_layer::WgpuTexture;
use wgpu_layer::*;
use winit::{
    event::*,
...
impl TextureRenderer {
    async fn new(window: &Window) -> Self {
        ...
        let diffuse_rgba = diffuse_image.as_rgba8().unwrap(); // panic

        use image::GenericImageView;
        let dimensions = diffuse_image.dimensions();
        let diffuse_texture =
            WgpuTexture::from_bytes(&device, &queue, diffuse_bytes, "happy-tree.png").unwrap();
